...
class camThread(threading.Thread):
    ...
    def run(self):
        ...
        print(f"Camera {self.camID} warming up...")
        time.sleep(4)  # Allow the camera to warm up
...
def eye_tracking(detector, predictor, cap, camID):
    try:
        while not stop_event.is_set():
            ...
            # if len(dets) == 0:
            #     frame_queue.put((f'Frame {camID}', camID, frame))
            #     continue
            ...
            for d in dets:              
                ...
                for d_rotated in dets_rotated:
                    ...
                    # frame_queue.put((f'Eyes {camID}', camID, resized_frame))
                    frame_queue.put((f'Mask Eye {camID}', camID, masked_eyes))
            ...
            # frame_queue.put((f'Frame {camID}', camID, frame))
            frame_queue.put((f'Mask {camID}', camID, masked_frame))
            # frame_queue.put((f'Inverse Mask {camID}', camID, inverse_masked_frame))
