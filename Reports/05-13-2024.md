...
* Got dlib facial recognition working with my CPU and not skipping too many frames. I then imported the facial landmark predictor which allows me to mark different poitns on ones face.

* Began looking into using multiple camera's to get ready for using a commercial eye tracker alongside a webcame if possible. Also I thought it would make it easier to track eyes independent from one another.
* Next step is to crop the camera to focus on my eyes instead. I will then want my left and right eyes and not the center of my face side by side. After doing so, I want to transform the image to simply be black and white and see if it is able to simply focus on my iris and pupil.   
