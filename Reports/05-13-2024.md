...
| 05/17 |  Worked on improving image segmentation with PyTorch and dealt with various technical issues.
...
- Improved image segmentation for my eye tracker by reading an article on using PyTorch for detecting eyes in low-quality images.
    - Inspired by Michael Reeves' video on tracking a moving fish using image segmentation, implemented a similar approach to understand masking and segmenting.
- Used facial landmark points to draw a mask over the eyes, turning everything else black to differentiate them.
- Attempted to use PyTorch for image segmentation to recognize the pupil's position but encountered issues running the code.
    - Faced problems creating a virtual environment and committing changes, requiring a hard reset and recovery of previous files.
    - Needed to train a model using a dataset available only on a Chinese website, which required a Chinese phone number for account creation.
- Dealt with frequent computer crashes and decided to end the day.
